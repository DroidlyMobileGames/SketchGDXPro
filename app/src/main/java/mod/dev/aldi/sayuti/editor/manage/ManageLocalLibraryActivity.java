...
public class ManageLocalLibraryActivity extends Activity implements View.OnClickListener, LibraryDownloader.OnCompleteListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().hasExtra("sc_id")) {
            ...
            local_lib_file = FileUtil.getExternalStorageDir().concat("/.sketchwaregames/data/").concat(sc_id.concat("/local_library"));
        }
    ...
    private void _copyFile(InputStream in, OutputStream out) {
        ...
        new ManageNativelibsActivity().unzip(local_libs_path + "libgdx-1.11.zip",local_libs_path);
    ...
    private void loadFiles() {
        ...
        if (!notAssociatedWithProject) {
            String fileContent;
            if (!FileUtil.isExistFile(local_lib_file) || (fileContent =
                    FileUtil.readFile(local_lib_file)).equals("")) {
