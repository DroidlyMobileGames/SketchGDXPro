...
import com.sketchware.remodgdx.xml.XmlBuilder;
...
public class Ix {
    ...
    public String a() {
        ...
        for (ProjectFileBean projectFileBean : b) {
            if (!projectFileBean.fileName.contains("_fragment")) {
                ...
                // javaName = projectFileBean.getJavaName();
        ...
        return AndroidManifestInjector.mHolder(a.toCode(), c.sc_id)
                .replaceAll("\\$\\{applicationId\\}", packageName);
    }
    ...
    private ArrayList<HashMap<String, Object>> getActivityAttrs() {
        String activityAttributesPath = FileUtil.getExternalStorageDir().concat("/.sketchwaregames/data/").concat(c.sc_id).concat("/Injection/androidmanifest/attributes.json");
